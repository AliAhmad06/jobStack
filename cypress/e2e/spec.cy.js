...
describe.only('Update profile', () => {
  ...
  it('Update Profile', () => {
    ...
    cy.get(':nth-child(6) > .flex > :nth-child(2) > .py-2').click()
    cy.get('.max-w-md > .relative > :nth-child(2)').click()
    cy.get('#company').clear().type('Orcalo')
    cy.get('#job_title').clear().type('New Job')
    cy.get('#start_date').click().contains('15').click()
    cy.get('#currentlyWorking').check()
    cy.get('#document').click()
    cy.get('.relative > :nth-child(2) > .justify-center > .flex').click()
  })
})
